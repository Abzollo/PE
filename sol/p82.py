...
def nbhr(i,j):
	if i > 0: yield (i-1,j)
	if i < 79: yield (i+1,j)
	#if j > 0: yield (i, j-1)  # Can't move backward in p082
	if j < 79: yield (i, j+1)

M = [[int(x) for x in line.split(",")] for line in open("p082_matrix.txt").read().splitlines()]
...
for i in xrange(80): makeEdge(G, (-1,-1), (i,0), M[i][0])
for i in xrange(80): makeEdge(G, (i,79), (80,80), 0)
